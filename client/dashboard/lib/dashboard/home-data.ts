...
export async function getDashboardHomeData(): Promise<DashboardHomeData> {
  const [recentResponse, pendingResponse, paidOrders, inventory] =
    await Promise.all([
      apiClient.get<DashboardOrder[]>("/orders?limit=5&page=1"),
      apiClient.get<DashboardOrder[]>(
        "/orders?status=PENDING_PAYMENT&limit=1&page=1",
      ),
      fetchAllPages<DashboardOrder>("/orders?paymentStatus=PAID"),
      fetchAllPages<DashboardInventoryStock>("/inventory"),
    ]);

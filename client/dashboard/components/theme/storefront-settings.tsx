...
export function StorefrontSettings() {
  ...
  const canRead = can("merchant.read") && can("theme.read");
  ...
  if (!canRead) {
    return <Notice text="Merchant and theme read permissions are required." />;
  }
...
function SettingsForm({
  ...
  const { can } = usePermissions();
  const canSave = can("merchant.update") && can("theme.update");
  ...
  return (
    ...
    >
      ...
      {!canSave && (
        <Notice text="Merchant update and theme update permissions are required to save." />
      )}

...
export function CheckoutPage({ sessionId }: { sessionId: string }) {
  ...
  useEffect(() => {
    if (selectedProvider || !checkoutQuery.data?.paymentProviders.length) {
      return;
    }

    setSelectedProvider(checkoutQuery.data.paymentProviders[0].provider);
  }, [checkoutQuery.data?.paymentProviders, selectedProvider]);
  ...
  const paymentProviders = checkout.paymentProviders;
            ...
            <div className="rounded-2xl border border-zinc-200 bg-white p-5 shadow-sm">
              ...
              {paymentProviders.length ? (
                <RadioGroup
                  ...
                  onChange={(value) =>
                    setSelectedProvider(value as PaymentProviderCode)
                  }
                >

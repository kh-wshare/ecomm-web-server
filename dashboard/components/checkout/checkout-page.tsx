...
import { useCheckoutSession } from "@/hooks/api/use-checkout";
import {
...
export function CheckoutPage({ sessionId }: { sessionId: string }) {
  ...
  const checkoutQuery = useCheckoutSession(
    sessionId,
    context?.token,
    Boolean(context),
  );
    ...
    return (
      <CheckoutNotice
        action={
          <Link
            className="rounded-full bg-zinc-950 px-5 py-2.5 text-sm font-semibold text-white"
            href={`/store/${context.merchantSlug}/products/${context.productSlug}`}
          >
            Return to product
          </Link>
        }
      ...
      <div className="mx-auto max-w-6xl">
        <header className="flex flex-wrap items-center justify-between gap-4">
          <Link
            ...
            href={`/store/${context.merchantSlug}`}
          >
            ← Continue shopping
          </Link>
        ...
        <div className="mt-8 grid gap-7 lg:grid-cols-[minmax(0,1fr)_380px]">
          <section className="space-y-6">
            ...
            <div className="rounded-2xl border border-zinc-200 bg-white p-5 shadow-sm">
              ...
              <label className="mt-4 flex cursor-pointer items-start gap-3 rounded-xl border border-zinc-950 bg-zinc-50 p-4">
                ...
                <span>
                  ...
                  <span className="mt-1 block text-xs leading-5 text-zinc-500">
                    Creates a protected payment reference. Settlement is
          ...
          <aside className="lg:sticky lg:top-8 lg:self-start">
            <div className="rounded-2xl border border-zinc-200 bg-white p-5 shadow-sm">
              ...
              <Button
                className="mt-6 h-12 w-full rounded-xl bg-zinc-950 px-5 text-sm font-bold text-white disabled:cursor-not-allowed disabled:opacity-50"
